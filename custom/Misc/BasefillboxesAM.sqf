_crate_BAM = _this select 0;

clearWeaponCargoGlobal _crate_BAM;
clearMagazineCargoGlobal _crate_BAM;

private ["_crate_BAM","_amount","_lootTable","_index","_itemType","_config","_itemTypes","_weights","_cntWeights","_num"];

_lootTable = "BASELOOT5";

crate_add_loot = {

	private ["_iItem","_iClass","_itemTypes","_index","_weights","_cntWeights","_qty","_max","_tQty","_canType","_mags","_amount","_crate_BAM"];
...

	{
...

		{
...

			{
				...
				_crate_BAM addMagazineCargoGlobal [_canType,_tQty];
...

			{
				...
				_crate_BAM addWeaponCargoGlobal [_iItem,random(4)];
...

		{
			...
			_crate_BAM addBackpackCargoGlobal [_iItem,1];
...

		{
			...
			_crate_BAM addWeaponCargoGlobal [_iItem,_amount];
...

			{
				...
				_crate_BAM addMagazineCargoGlobal [(_mags select 0), (_amount*(round(random 5)))];
...

		{
			...
			//_crate_BAM addWeaponCargoGlobal [_iItem,1];
...
for "_x" from 1 to (_num + _amount) do {
	...
	[_itemType select 0, _itemType select 1, _crate_BAM] spawn crate_add_loot;
};