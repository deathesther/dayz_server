/*
	MAI Configuration File
	
	Description: Contains all configurable settings of MAI. Contains settings for debugging, customization of AI units, spawning, and loot.
	
	Last updated: 1:02 AM 11/4/2013
*/

diag_log "[MAI] Reading MAI configuration file.";
...
/*
	...
	MAI_modName value		Enables extra features (ie: Items, AI skins, loot rates, etc.) for...
	--------------------------------------------------------------------------------------------------------------------
	""						Automatically detect mod (can be manually specified by editing MAI_modName below)
	"default"				Force default settings
	"2017"					DayZ 2017/Namalsk 2017	(Can't be automatically detected, must manually set MAI_modName = "2017" to enable)
	"epoch"					DayZ Epoch 				(Automatically detected - no need to edit)
	"civilian"				DayZ Civilian 			(Can't be automatically detected, must manually set MAI_modName = "civilian" to enable)
...
//AI weapon noise multiplier for zombie aggro purposes. No effect if MAI_zombieEnemy is set to false. Enabling this option may impact server performance as a script is run for each AI bullet fired.
//Note: AI cannot be attacked or damaged by zombies.(Default: 0.00. Player equivalent: 1.00)		
MAI_weaponNoise = 0.00;

//Passive zombie-to-AI aggro. If enabled, zombies will tend to be attracted to a nearby AI group leader. Less impactful on server performance than setting a value to MAI_weaponNoise (Default: false).
MAI_passiveAggro = false;
...
MAI_maxHeliPatrols = 3;

//Set minimum and maximum wait time in seconds to respawn an AI vehicle patrol after vehicle is destroyed or disabled. (Default: Min 600, Max 900).
MAI_respawnTMinA = 600;
MAI_respawnTMaxA = 900;
...
MAI_heliTypes = ["UH60M_EP1_DZ"];	

//Specify vehicle weapon for air vehicles that are unarmed by default. MAI will arm these air vehicles with the specified weapons upon spawning each vehicle.
//NOTE: These classnames are not verified by MAI - it is the user's responsibility to make sure they are valid and unbanned.
...
MAI_maxLandPatrols = 3;

//Set minimum and maximum wait time in seconds to respawn an AI vehicle patrol after vehicle is destroyed or disabled. (Default: Min 600, Max 900).
MAI_respawnTMinL = 600;
MAI_respawnTMaxL = 900;

//Classnames of land vehicle types to use. (Default: "UAZ_Unarmed_TK_EP1")
MAI_vehTypes = ["HMMWV_M1035_DES_EP1","HMMWV_M1151_M2_CZ_DES_EP1_DZ","HMMWV_M998A2_SOV_DES_EP1_DZ"];
...
//Highly recommended to enable MAI_verifyTables if this option is set to false. 
MAI_dynamicWeaponList = true;

//(Only if MAI_dynamicWeaponList = true) List of classnames of weapons that AI should never use. By default, AI may carry any lootable weapon. 
//Example: MAI_banAIWeapons = ["M107_DZ","BAF_AS50_scoped"] will remove the M107 and AS50 from AI weapon tables if dynamic weapon list is enabled.								
...
//Weapon classnames added here are verified if MAI_verifyTables is set to true.
MAI_launcherTypes = [];	
...
//Maximum number of items to select from MAI_MiscItemS table.										
MAI_numMiscItemS = 3;						

//Maximum number of items to select from MAI_MiscItemL table.				
MAI_numMiscItemL = 2;										
...
//Note: Additional AI skill settings can be defined (MAI_skill4 - MAI_skill9) using the same format above.
//Instructions: replace "nil" with the skill array. Refer to the above preset skill arrays for examples.
//Custom AI skill settings can only be used with custom-defined spawns (spawns created using the MAI_spawn function).
...
//Load custom MAI settings file.
call compile preprocessFileLineNumbers "\z\addons\dayz_server\MAI\MAI_settings_override.sqf";

diag_log "[MAI] MAI configuration file loaded.";
