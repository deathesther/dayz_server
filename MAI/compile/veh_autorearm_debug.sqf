...

{
	if (_x in ["CarHorn","BikeHorn","TruckHorn","TruckHorn2","SportCarHorn","MiniCarHorn"]) then {
		if (MAI_debugLevel > 0) then {diag_log format ["MAI Debug: Removed vehicle weapon %1 from AI group %2 vehicle %3.",_x,_unitGroup,typeOf _vehicle];};
		_vehWeapons = _vehWeapons - [_x];
	};
} forEach _vehWeapons;

if (MAI_debugLevel > 0) then {diag_log format ["MAI Debug: Vehicle %1 driver is %2. Crew is %3. Vehicle weapons: %4.",(typeOf _vehicle),(driver _vehicle),(crew _vehicle),_vehWeapons];};
_startTime = time;
	...
	while {_vehOK && _driverOK} do {	
		//Check if Vehicle ammunition needs to be replenished
		{
			if ((_vehicle ammo _x) < 20) then {
				...
				if (MAI_debugLevel > 1) then {diag_log "MAI Extended Debug: Reloaded ammo for AI patrol Vehicle.";};
			};
		} forEach _vehWeapons;
		...
		if (fuel _vehicle < 0.20) then {
			_vehicle setFuel 1;
			if (MAI_debugLevel > 1) then {diag_log "MAI Extended Debug: Refueled AI patrol Vehicle.";};
		};
...
if (MAI_debugLevel > 0) then {diag_log format ["MAI Debug: AI Vehicle patrol disabled at %1 after %2 seconds of flight.",(getPosATL _vehicle),_timePatrolled];};
if (_timePatrolled < 35) then {
	diag_log "MAI Warning: An AI Vehicle was disabled less than 35 seconds after being spawned. Please check if server_cleanup.fsm was edited properly.";
};
