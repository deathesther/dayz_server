/*
	...
	Description: Spawns a group of AI units. Used for spawning of MAI's static, dynamic, and custom AI units.
...
_unitGroup = if (isNull (_this select 1)) then {createGroup (call MAI_getFreeSide)} else {_this select 1};
...
if (MAI_debugLevel > 1) then {diag_log format ["MAI Extended Debug: Found spawn position at %3 meters away at position %1 after %2 attempts.",_pos,_attempts,(_pos distance _spawnPos)]};
