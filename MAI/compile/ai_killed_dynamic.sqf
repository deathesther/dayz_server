...
if (_unitsAlive == 0) then {
	if (MAI_debugLevel > 0) then {diag_log format["MAI Debug: All units in group %1 are dead. Spawning temporary dummy unit for group. (fnc_dynAIDeathmil).",_unitGroup];};
	...
	if (MAI_debugLevel > 0) then {diag_log format["MAI Debug: All groups spawned by trigger %1 are dead. Starting force respawn. (fnc_dynAIDeathmil).",_trigger];};
...
if (MAI_debugLevel > 0) then {diag_log format["MAI Debug: AI group %1 killed, %2 units left alive in group. (fnc_dynAIDeathmil).",_unitGroup,_unitsAlive];};
