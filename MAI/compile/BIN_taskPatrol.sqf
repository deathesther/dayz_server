...
if (isServer) then
{
	...
	//_grp setBehaviour _MAI_behavior;
	_grp setBehaviour "AWARE";
	//_grp setSpeedMode (["FULL","NORMAL"] call BIS_fnc_selectRandom3);
	_grp setSpeedMode "FULL";
	//_grp setCombatMode (["YELLOW", "RED"] call BIS_fnc_selectRandom3);
	_grp setCombatMode "RED";
	//_grp setFormation (["STAG COLUMN", "WEDGE", "ECH LEFT", "ECH RIGHT", "VEE", "DIAMOND"] call BIS_fnc_selectRandom3);
	...
	while {count _wp_array < _wp_count} do 
	{
		...
		_b = 0 + (_wp_pos select 1);
			...
			if(count _bldgpos != 0) then {_wp_pos = _bldgpos call BIS_fnc_selectRandom3;};
	...
	for "_i" from 1 to (_wp_count - 1) do
	{
		...
		if (!(surfaceIsWater _cur_pos)) then {
			...
			_wp setWaypointStatements ["true", "if ((random 3) > 2) then { group this setCurrentWaypoint [(group this), (floor (random (count (waypoints (group this)))))];} else {_nul = [(group this),100] spawn MAI_findLootPile;};"];
