...
/*
	Damage Handler script modified for MAI
...
if ((group _unit) == (group _dmgSource)) then {_damage = (_damage/5)};	//Reduce friendly fire and collision damage.
if (((_damage > 2) || ((_damage > 0.5) && (_selection == "head_hit"))) && !(_unit getVariable ["unconscious",false])) then {_nul = [_unit] spawn MAI_unconscious; _unit setVariable ["unconscious",true];};
...
switch (_selection) do {                                                                                           		// Depending on which part of body is damaged
    case "":{                                                                                                         	// Overall structure damage
        _damage = (damage _unit) + _damage * (MAI_dmgFactors select 0)                                             	// Total damage to unit + percentage of incoming damage
    };
    
    case "head_hit":{                                                                                                	// damage applied to head
        _damage = (_gethit select 0) + (_damage - (_gethit select 0))*(MAI_dmgFactors select 1);
    ...
    case "body":{
        _damage = (_gethit select 1) + (_damage - (_gethit select 1))*(MAI_dmgFactors select 2);
		_gethit set [1,_damage]        //As previously
    };    
    
    case "hands":{                                                                                                      // cannot kill a unit on own, only affect aim
        _damage = (_gethit select 2) + (_damage - (_gethit select 2))*(MAI_dmgFactors select 3);
		_gethit set [2,_damage];
		//if (_damage < 1) then {_damage = 0};
    };        
    
    case "legs":{                                                                                                      // cannot kill a unit on own, only affect movement
        _damage = (_gethit select 3) + (_damage - (_gethit select 3))*(MAI_dmgFactors select 4);
