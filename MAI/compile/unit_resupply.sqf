...
if ((((units _unitGroup) select 0) == _unit) && (_weapongrade in MAI_launcherLevels) && ((count MAI_launcherTypes) > 0)) then {
	private ["_launchWeapon","_launchAmmo"];
	_launchWeapon = MAI_launcherTypes call BIS_fnc_selectRandom2;
	...
	if !(_unit getVariable ["unconscious",false]) then {
		for "_i" from 0 to ((count _unitWeapons) -1) do {
			...
			if ((_unit ammo (_unitWeapons select _i) == 0) || (_needsReload))  then {		//If active weapon has no ammunition, or AI has no magazines, remove empty magazines and add a new magazine.
				...
				if (MAI_debugLevel > 1) then {diag_log format ["MAI Extended Debug: AI ammo depleted, added magazine %1 to AI %2.",(_unitMagazines select _i),_unit];};
			};
		};
		if (_bandages > 0) then {
			private ["_health"];
