...
if (_damage > 0.1) then {
	...
	if (((_damage > 2) || ((_damage > 0.5) && (_hit == "head_hit"))) && !(_unit getVariable ["unconscious",false])) then {_nul = [_unit,_hit] spawn MAI_unconscious; _unit setVariable ["unconscious",true];};
};

if (_newbloodlevel <= 0) then {
	_nul = [_unit,_source] call MAI_unitDeath;
};
