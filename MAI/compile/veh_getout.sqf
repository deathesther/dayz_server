...
//Convert vehicle units to ground units
{
	if (alive _x) then {
		private ["_health"];
		0 = [_x, 3] spawn MAI_autoRearm_unit;
...
if (MAI_debugLevel > 0) then {diag_log format ["MAI Debug: AI vehicle %1 evacuated at %2.",typeOf _vehicle,mapGridPosition _vehicle];};
