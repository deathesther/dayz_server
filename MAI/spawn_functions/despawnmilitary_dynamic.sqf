...
if (_forceDespawn) then {
	_waitTime = MAI_dynRemoveDeadWait;	//Longer sleep if despawning due to all units killed to allow for looting.
} else {
	_waitTime = MAI_dynDespawnWait;	//Shorter sleep if despawning due to no player presence.
};
...
if (MAI_debugLevel > 1) then {diag_log format["MAI Extended Debug: No players remain in trigger area. Deleting spawned AI in %1 seconds.",_waitTime];};
...
if (MAI_debugLevel > 1) then {diag_log "MAI Extended Debug: Relocating dynamic trigger.";};
...
if (MAI_debugLevel > 0) then {diag_log format ["MAI Debug: Despawned AI in dynamic trigger area at %1. Trigger relocated to %2.",_oldPos,(mapGridPosition _trigger)];};
