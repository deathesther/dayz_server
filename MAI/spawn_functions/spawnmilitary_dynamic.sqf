...
if (surfaceIsWater (getPosATL _trigger)) exitWith {
	...
	if (MAI_debugLevel > 1) then {diag_log format ["MAI Extended Debug: Could not find suitable location to spawn AI units, relocating trigger to position %1. (spawnmilitary_dynamic)",_newPos];};
};

//Reduce number of AI spawned if trigger area intersects another activated trigger to avoid overwhelming AI spawns.
_nearbyTriggers = ({((_trigger distance _x) < (((triggerArea _trigger) select 0) - (((triggerArea _trigger) select 0) * MAI_dynOverlap)))&&(triggerActivated _x)} count MAI_dynTriggerArray) - 1;
if (_nearbyTriggers > 0) exitWith {
	_newPos = _trigger call MAI_relocDynTrigger;
	if (MAI_debugLevel > 1) then {diag_log format ["MAI Extended Debug: Dynamic trigger intersects with another active trigger, relocating trigger to position %1. (spawnmilitary_dynamic)",_newPos];};
};
...
if (MAI_debugLevel > 1) then {diag_log format ["MAI Extended Debug: Counted %1 players within 100m of target player (spawnmilitary_dynamic)",_playerCount];};
...
if (_findPlayer) then {
	...
	0 = [_unitGroup,_spawnPos,_patrolDist,MAI_debugMarkers] spawn MAI_BIN_taskPatrol;
