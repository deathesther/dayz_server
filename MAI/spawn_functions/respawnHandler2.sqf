/*
	...
	Usage: [_trigger,_unitGroup] spawn fnc_respawnHandler_M (for infantry-type AI units), ["AIR] spawn fnc_respawnHandler_M (for Air-type AI patrols), ["land"] spawn fnc_respawnHandler_M (for Land-type AI patrols)
...
if ((count _this) > 1) then {
	...
	_respawnSleep = switch (toLower _patrolType) do {
		case "air": {(MAI_respawnTMinA + random (MAI_respawnTMaxA - MAI_respawnTMinA))};
		case "land": {(MAI_respawnTMinL + random (MAI_respawnTMaxL - MAI_respawnTMinL))};
		case default {600};
	};
	_nextRespawnTime = (time + _respawnSleep);	//Determine time of next respawn
	MAI_respawnQueue set [(count MAI_respawnQueue),[time + _respawnSleep,_patrolType]];
	if (MAI_debugLevel > 0) then {diag_log format ["MAI Debug: Added AI vehicle patrol type %1 to respawn queue. Queue position %2. Wait Time %3 (respawnHandler)",_patrolType,(count MAI_respawnQueue),_respawnSleep];};
};
...
if (!isNil "MAI_nextRespawnTime") then {
	if (_nextRespawnTime < MAI_nextRespawnTime) then {	//If the newest respawn is scheduled to happen sooner than the next closest respawn, reduce the initial wait time appropriately.
		...
		if (MAI_debugLevel > 1) then {diag_log format ["MAI Extended Debug: Decreased time to next respawn to %1 seconds.",_respawnSleep];};
	};
} else {
	MAI_nextRespawnTime = _nextRespawnTime;
	if (MAI_debugLevel > 1) then {diag_log format ["MAI Extended Debug: Time to first respawn set to %1 seconds.",_respawnSleep];};
};
...
while {time < MAI_nextRespawnTime} do {				//Check if it's time to process the first respawn (5 checks per minute).
	sleep 12;
};

MAI_respawnActive = true;							//First respawn is now being processed, so deny subsequent attempts to modify the initial wait time.
MAI_queueActive = nil;
			...
			switch (typeName _trigger) do {
				...
				case "STRING": {
					if ((toLower _trigger) == "air") then {
						_helipatrols = 1 spawn MAI_spawnHeliPatrol;
					} else {
						if ((toLower _trigger) == "land") then {
							_vehpatrols = 1 spawn MAI_spawnVehPatrol;
						};
					};
				};
			};
