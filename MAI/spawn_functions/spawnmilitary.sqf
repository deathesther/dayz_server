/*
	...
	Usage: [_minAI, _addAI, _patrolDist, _trigger, _numGroups (optional)] spawn spawnmilitary;
...
_gradeChances = [_equipType] call MAI_getGradeChances;
...
if ((count _locationArray) == 0) then {
	...
	if ((count _positionArray) == 0) then {
		...
		if (MAI_debugLevel > 1) then {diag_log "MAI Extended Debug: Spawning AI from building positions (spawnmilitary).";};
	} else {
		if (typeName (_positionArray select 0) == "STRING") then {
			...
			if (MAI_debugLevel > 1) then {diag_log "MAI Extended Debug: Spawning AI from marker positions (spawnmilitary).";};
		};
	};
} else {
	if (MAI_debugMarkers > 0) then {
	...
	if (MAI_debugLevel > 1) then {diag_log "MAI Extended Debug: Spawning AI from stored positions (spawnmilitary).";};
};
...
for "_j" from 1 to _numGroups do {
	...
	if (_totalAI > 0) then {
		...
		_nul = [_trigger] spawn MAI_retrySpawn;
		if (MAI_debugLevel > 0) then {diag_log format ["MAI Debug: No AI to spawn for AI group %1 of %2.",_j,_numGroups]};
	};
};

if (MAI_debugLevel > 0) then {diag_log format["MAI Debug: Spawned %1 new AI groups (%2 units total) in %3 seconds at %4 (spawnmilitary).",_numGroups,_totalSpawned,(diag_tickTime - _startTime),(triggerText _trigger)];};

0 = [_trigger,_grpArray,_patrolDist,_gradeChances,_spawnPositions,[_minAI,_addAI]] call MAI_setTrigVars;
