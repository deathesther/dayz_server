...
/*
	verifyTables script written for MAI
...
waitUntil {!isNil "MAI_weaponsInitialized"};	//Wait for MAI to finish building weapon classname arrays.
...
diag_log "[MAI] MAI is verifying all tables for banned or invalid classnames...";
{
...
if (_errorFound) then { //If a classname table requires editing, perform the required edits then report their final states.
	...
	diag_log "[MAI] All tables have been verified. No invalid entries found.";
};
diag_log format ["[MAI] Table verification completed in %1 seconds.",(diag_tickTime - _startTime)];

MAI_classnamesVerified = true;
