...
_heliEnabled = (MAI_maxHeliPatrols > 0);
_vehEnabled = (MAI_maxLandPatrols > 0);
sleep 60;
...
while {true} do {
	...
	if (MAI_dynAISpawns || _heliEnabled || _vehEnabled) then {diag_log format ["MAI Monitor :: Dynamic Spawns - %1/%2 (active/total). Air Patrols: %3/%4 (cur/max). Land Patrols: %5/%6.",MAI_actDynTrigs,MAI_curDynTrigs,MAI_curHeliPatrols,MAI_maxHeliPatrols,MAI_curLandPatrols,MAI_maxLandPatrols];};
