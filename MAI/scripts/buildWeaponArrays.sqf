...
if (!isNil "MAI_weaponsInitialized") exitWith {};

_startTime = diag_tickTime;
diag_log "[MAI] Building MAI weapon arrays using CfgBuildingLoot data.";
...
//diag_log format ["DEBUG :: List of weapons to be removed from MAI classname tables: %1",_aiWeaponBanList];
			...
			if ((_lootItem select 1) == "weapon") then {			//Build an array of "weapons", then categorize them as rifles or pistols, then sort them into the correct weapon grade.
				...
				if ((_weaponMags > 0) && !(_weaponItem in _aiWeaponBanList)) then {							//Consider an item as a "weapon" if it has at least one magazine type.
					if ((getNumber (configFile >> "CfgWeapons" >> _weaponItem >> "type")) == 1) then {
						call compile format ["MAI_Rifles%1 set [(count MAI_Rifles%1),'%2'];",_i,_weaponItem];
					} else {
...
if ((count MAI_Pistols3) == 0) then {
	diag_log "MAI_Pistols3 is empty. Populating with entries from MAI_Pistols2.";
	MAI_Pistols3 = [] + MAI_Pistols2;
};

//In case the mod has no HeliCrash loot tables...
if ((count MAI_Rifles3) == 0) then {
	diag_log "MAI_Rifles3 is empty. Populating with entries from MAI_Rifles2.";
	MAI_Rifles3 = [] + MAI_Rifles2;
};

if (MAI_debugLevel > 0) then {
	//Display finished weapon arrays
	diag_log format ["Contents of MAI_Pistols0: %1",MAI_Pistols0];
	diag_log format ["Contents of MAI_Pistols1: %1",MAI_Pistols1];
	diag_log format ["Contents of MAI_Pistols2: %1",MAI_Pistols2];
	diag_log format ["Contents of MAI_Pistols3: %1",MAI_Pistols3];

	diag_log format ["Contents of MAI_Rifles0: %1",MAI_Rifles0];
	diag_log format ["Contents of MAI_Rifles1: %1",MAI_Rifles1];
	diag_log format ["Contents of MAI_Rifles2: %1",MAI_Rifles2];
	diag_log format ["Contents of MAI_Rifles3: %1",MAI_Rifles3];
};

diag_log format ["[MAI] MAI weapon classname tables created in %1 seconds.",(diag_tickTime - _startTime)];
