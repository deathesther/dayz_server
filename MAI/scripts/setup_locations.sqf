...
if (isNil "MAI_newMap") then {
...

	{
		...
		if (_placeType in ["NameCityCapital","NameCity","NameVillage","NameLocal"]) then {
			...
			MAI_locations set [(count MAI_locations),[_placeName,_placePos,_placeType,objNull]];	//Location Name, Position, Type, Dynamic Trigger object (objNull if none).
			...
			if (_placeType in ["NameCityCapital","NameCity","NameVillage","NameLocal"]) then {
				...
				if (MAI_staticAI) then {
					if (_placeType != "NameLocal") then {
						...
						_statements = format ["0 = [%1,%2,%3,thisTrigger,[],%4] call fnc_spawnmilitary;",_aiCount select 0,_aiCount select 1,_patrolRad,_equipType];
						_trigger setTriggerStatements ["{isPlayer _x} count thisList > 0;", _statements, "0 = [thisTrigger] spawn fnc_despawnmilitary;"];
					};
				};
...
MAI_locations_ready = true;

diag_log format ["[MAI] %1 locations gathered in %2 seconds.",_locCount,(diag_tickTime - _startTime)];
